...
@interface CircleVC ()<UITableViewDelegate,UITableViewDataSource,XLPhotoBrowserDelegate,XLPhotoBrowserDatasource,UIActionSheetDelegate>
{
    NSInteger page;
...
- (UITableView *)myTableView{
    if (!_myTableView) {
        ...
        [_myTableView registerNib:[UINib nibWithNibName:@"CircleCell" bundle:nil] forCellReuseIdentifier:@"CircleCell"];
    }
    return _myTableView;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString * cell = @"CircleCell";
    CircleCell *myCell = [tableView dequeueReusableCellWithIdentifier:cell forIndexPath:indexPath];
    if (!myCell) {
    ...
    if (_circleDataList) {
        ...
        myCell.likeBtnBlock = ^(NSInteger integer, CircleCell *onceCell) {
            likeCell = onceCell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    CircleDataList *dict = _circleDataList[indexPath.row];
    CircleDetailVC *page = [[CircleDetailVC alloc]init];
    page.dict = dict;
    page.headerHeight = [self tableView:self.myTableView heightForRowAtIndexPath:indexPath];
...
- (UIImageView *)photoBrowser:(XLPhotoBrowser *)browser sourceImageViewForIndex:(NSInteger)index
{
    ...
    CircleCell *myCell = [_myTableView cellForRowAtIndexPath:indexPath];
    if (totolPicNum == 1) {
