//
//  MyProperyServiceClassTableViewController.m
//  jujiahe
//
...
@interface MyProperyServiceClassTableViewController ()<UITableViewDelegate,UITableViewDataSource,XLPhotoBrowserDelegate,XLPhotoBrowserDatasource,UIActionSheetDelegate>
{
    NSInteger pageNum;
...
@implementation MyProperyServiceClassTableViewController
...
- (void)fetchData:(NSInteger)page{
    ...
    [ZTHttpTool postWithUrl:@"property/v1/PropertyService/infos" param:dict success:^(id responseObj) {
        ...
        if (data.rcode == 0) {
            ...
            if (data.form.count>0) {
                if (pageNum == 1) {
                    ...
                    [UIView performWithoutAnimation:^{
                        ...
                        [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:_myArr.count - data.form.count - 1 inSection:0] atScrollPosition:UITableViewScrollPositionNone animated:NO];
                    }];
                }
            }else{
                if (page>1) {
            ...
            if (_myArr.count ==0) {
                [self.tableView addEmptyViewWithImageName:@"暂无记录" title:@"暂无记录"];
...
- (UIImageView *)photoBrowser:(XLPhotoBrowser *)browser sourceImageViewForIndex:(NSInteger)index
{
    ...
    ServiceCell *myCell = [self.tableView cellForRowAtIndexPath:indexPath];
