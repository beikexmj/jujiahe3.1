...
- (void)initialization
{
    ...
    _pageDotColor = RGBA(0xffffff, 0.5);
...
- (void)setImagePathsGroup:(NSArray *)imagePathsGroup
{
    ...
    if (imagePathsGroup.count > 1) { // 由于 !=1 包含count == 0等情况
        ...
        if (imagePathsGroup.count == 1) {
            NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:0 target:self selector:@selector(automaticScroll) userInfo:nil repeats:NO];
            _timer = timer;
