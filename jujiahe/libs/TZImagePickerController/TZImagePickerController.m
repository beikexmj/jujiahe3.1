...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate allowPickingVideo:(BOOL)allowPickingVideo{
    TZAlbumPickerController *albumPickerVc = [[TZAlbumPickerController alloc] init];
    self = [super initWithRootViewController:albumPickerVc];
    if (self) {
        self.maxImagesCount = maxImagesCount; //> 0 ? maxImagesCount : (allowPickingVideo ? 1 : 9 ); // Default is 9 / 默认最大可选9张图片
...
- (void)configTableView {
    ...
    [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo completion:^(NSArray<TZAlbumModel *> *models) {
        ...
        CGFloat top = 44;
        if (iOS7Later) top += 20;
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, top, SCREENWIDTH, SCREENHEIGHT - top) style:UITableViewStylePlain];
