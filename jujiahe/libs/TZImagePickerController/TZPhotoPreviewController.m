...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_currentIndex) [_collectionView setContentOffset:CGPointMake((self.view.frame.size.width) * _currentIndex, 0) animated:NO];
...
- (void)configCustomNaviBar {
    _naviBar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 64 + (kDevice_Is_iPhoneX?22:0))];
    _naviBar.backgroundColor = [UIColor colorWithRed:(34/255.0) green:(34/255.0)  blue:(34/255.0) alpha:1.0];
    _naviBar.alpha = 0.7;
    
    _backButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 10+ kDevice_Is_iPhoneX?22:0, 44, 44)];
    [_backButton setImage:[UIImage imageNamed:@"navi_back"] forState:UIControlStateNormal];
    [_backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_backButton addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    
    _selectButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 54, 10+ kDevice_Is_iPhoneX?22:0, 42, 42)];
...
- (void)configBottomToolBar {
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 44 - (kDevice_Is_iPhoneX?34:0), self.view.frame.size.width, 44+(kDevice_Is_iPhoneX?34:0))];
    ...
    _okButton.frame = CGRectMake(self.view.frame.size.width - 44 - 12, 0, 44, 44);
    ...
    _numberImageView.frame = CGRectMake(self.view.frame.size.width - 56 - 24, 9, 26, 26);
...
- (void)configCollectionView {
    ...
    _collectionView.contentSize = CGSizeMake(self.view.frame.size.width * _photoArr.count, self.view.frame.size.height);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    _currentIndex = offSet.x / self.view.frame.size.width;
}
