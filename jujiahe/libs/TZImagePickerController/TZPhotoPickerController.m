...
- (void)configCollectionView {
    ...
    CGFloat top = margin + 44 + (kDevice_Is_iPhoneX?22:0);
    if (iOS7Later) top += 20;
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(margin, top, self.view.frame.size.width - 2 * margin, self.view.frame.size.height - 50 - (kDevice_Is_iPhoneX?34:0) - top) collectionViewLayout:layout];
    ...
    _collectionView.contentSize = CGSizeMake(self.view.frame.size.width, ((_model.count + 3) / 4) * self.view.frame.size.width);
...
- (void)configBottomToolBar {
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 50-(kDevice_Is_iPhoneX?34:0), self.view.frame.size.width, 50 +(kDevice_Is_iPhoneX?34:0))];
    ...
    if (imagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.frame = CGRectMake(50, self.view.frame.size.height - 50 - (kDevice_Is_iPhoneX?34:0), 130, 50);
    ...
    _okButton.frame = CGRectMake(self.view.frame.size.width - 44 - 12, 3, 44, 44);
    ...
    _numberImageView.frame = CGRectMake(self.view.frame.size.width - 56 - 24, 12, 26, 26);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo) {
        if (_selectedPhotoArr.count > 0) {
            ...
            if ([timeArr.lastObject integerValue]>10.0) {
                [MBProgressHUD showError:@"选择的视频长度不能超过10秒"];
                return;
            }
            TZVideoPlayerController *videoPlayerVc = [[TZVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            videoPlayerVc.conFromFlag = 0;
