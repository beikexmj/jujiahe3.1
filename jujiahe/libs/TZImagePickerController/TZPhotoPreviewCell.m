...
- (void)resizeSubviews {
    ...
    _imageContainerView.tz_width = self.frame.size.width;
    
    UIImage *image = _imageView.image;
    if (image.size.height / image.size.width > self.frame.size.height / self.frame.size.width) {
        _imageContainerView.tz_height = floor(image.size.height / (image.size.width / self.frame.size.width));
    } else {
        ...
        _imageContainerView.tz_centerY = self.frame.size.height / 2;
    }
    if (_imageContainerView.tz_height > self.frame.size.height && _imageContainerView.tz_height - self.frame.size.height <= 1) {
        _imageContainerView.tz_height = self.frame.size.height;
    }
    _scrollView.contentSize = CGSizeMake(self.frame.size.width, MAX(_imageContainerView.tz_height, self.frame.size.height));
    [_scrollView scrollRectToVisible:self.bounds animated:NO];
    _scrollView.alwaysBounceVertical = _imageContainerView.tz_height <= self.frame.size.height ? NO : YES;
