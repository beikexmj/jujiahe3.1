...
-(void)setupWithPasswordAlertViewType_default{
    ...
    self.bgView = [[UIView alloc]initWithFrame:CGRectMake(20, self.center.y-100, ScreenWidth-40, 275)];
    ...
    self.titleLable.textColor = RGBA(0x303030, 1);
    ...
    [cancleBtn setImage:[UIImage imageNamed:@"icon_back_gray"] forState:UIControlStateNormal];
    ...
    //金额
    self.moneyLable = [[UILabel alloc]initWithFrame:CGRectMake(40, CGRectGetMaxY(line1.frame)+25, self.bgView.frame.size.width-80, 50)];
    self.moneyLable.textAlignment = NSTextAlignmentCenter;
    self.moneyLable.font = [UIFont boldSystemFontOfSize:25.0];
    self.moneyLable.text = @"￥0.00";
    self.moneyLable.textColor = RGBA(0x303030, 1);
    [self.bgView addSubview:self.moneyLable];
    
    //开始绘制输入框(6个格子40X45)
    _inputBgView = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetWidth(self.bgView.frame)/2-135, CGRectGetMaxY(self.moneyLable.frame)+25, 270, 40)];
...
-(void)setupithPasswordAlertViewType_sheet{
    ...
    self.bgView = [[UIView alloc]initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 275)];
    self.bgView.backgroundColor = [UIColor whiteColor];
    [self addSubview:self.bgView];
    ...
    //取消按钮
    UIButton *cancleBtn  = [UIButton buttonWithType:UIButtonTypeCustom];
    cancleBtn.frame = CGRectMake(5, 10, 30, 30);
    cancleBtn.backgroundColor = [UIColor clearColor];
    [cancleBtn setImage:[UIImage imageNamed:@"icon_back_gray"] forState:UIControlStateNormal];
    [cancleBtn addTarget:self action:@selector(cancleBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.bgView addSubview:cancleBtn];
    ...
    for (int i = 0; i<6; i++) {
        ...
        UIImageView *smallImageView = [[UIImageView alloc]initWithFrame:CGRectMake(17.5, 15, 10, 10)];
        smallImageView.image = [UIImage imageNamed:@"passwordIcon"];
        [imgView addSubview:smallImageView];
    }
    ...
    [forgetPWBtn setTitleColor:RGBA(0x00a7ff, 1) forState:UIControlStateNormal];
    [forgetPWBtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    forgetPWBtn.tag = 1000;
...
-(void)keyBoardWillHidden{
    ...
    if (_currentType == PasswordAlertViewType_sheet) {
        [UIView animateWithDuration:0.25 animations:^{
            ...
            self.bgView.frame = CGRectMake(0, ScreenHeight, ScreenWidth, 275);
...
-(void)showInView:(UIView *)view{
    if (_flag) {
        [[self.bgView viewWithTag:1000] removeFromSuperview];
    }
